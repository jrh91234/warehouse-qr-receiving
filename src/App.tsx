...
import { Box, ScanLine, History, Settings, ArrowUpRight, ArrowRight, Camera, ImagePlus, Keyboard, Check, X, Wifi, WifiOff, RotateCw, PackageCheck, ChevronRight, CircleAlert, ScanQrCode, LoaderCircle, LockKeyhole, Users, UserPlus, ShieldCheck, LogOut, Trash2, Pencil, Filter } from 'lucide-react';
...
function dateKey(value:string){const parsed=new Date(value);return Number.isFinite(parsed.getTime())?parsed.toLocaleDateString('en-CA'):'';}
...
export default function App(){
  ...
  const [historyRecords,setHistoryRecords]=useState<Receipt[]>([]);
  ...
  useEffect(()=>{if(view==='history'&&user&&!historyLoaded&&validApiUrl(config.apiUrl))void loadHistory();},[view,user?.username,historyLoaded,config.apiUrl]);
  ...
  async function logout(){
    ...
    sessionStorage.removeItem(KEY+'-session');sessionStorage.removeItem(KEY+'-user');setSessionToken('');setUser(null);setView('scan');setRawQR('');setEditingRecord(null);setHistoryRecords([]);setHistoryLoaded(false);setHistoryStartDate(today);setHistoryEndDate(today);setHistoryEmployee('');setNotice(null);
  }
  ...
  async function loadHistory(){
    if(!user||!validApiUrl(config.apiUrl))return;
    setHistoryLoading(true);
    try{
      const result=await request(config.apiUrl,{action:'listReceipts'});
      const remote=Array.isArray(result.receipts)?result.receipts.map((item:Receipt)=>({...item,apiUrl:config.apiUrl,status:'synced' as const})):[];
      setHistoryRecords(remote);setHistoryLoaded(true);
    }catch(e){setHistoryLoaded(true);flash((e as Error).message,'error');}
  ...
  async function submit(event:React.FormEvent){
    ...
    const receipt:Receipt=editingRecord
      ? {...editingRecord,...fields,employee:editingRecord.employee,rawQR:editingRecord.rawQR,apiUrl:editingRecord.apiUrl||config.apiUrl,error:undefined}
      : {...fields,id:crypto.randomUUID(),rawQR,employee:user?.username||'',createdAt:new Date().toISOString(),status:'pending',apiUrl:config.apiUrl};
    ...
    if(editingRecord){
      ...
      try{
        if(current.status==='pending'){
          const next=recordRef.current.map(item=>item.id===current.id?{...receipt,status:'pending' as const}:item);
          saveRecords(next);
        }else{
          const result=await request(current.apiUrl||config.apiUrl,{action:'updateReceipt',receipt});
          const next=recordRef.current.map(item=>item.id===current.id?{...receipt,status:'synced' as const,serverTime:result.serverTime||item.serverTime}:item);
          saveRecords(next);setHistoryRecords(previous=>previous.map(item=>item.id===receipt.id?{...receipt,status:'synced' as const,serverTime:result.serverTime||item.serverTime}:item));
        }
        put(KEY+'-destination',{warehouse:fields.warehouse,location:fields.location});
        setEditingRecord(null);setRawQR('');setRecognized(false);setFields({...empty,warehouse:fields.warehouse,location:fields.location});
        flash(current.status==='pending'?'แก้ไขรายการแล้ว รายการจะถูกส่งเมื่อออนไลน์':'แก้ไขรายการรับเข้าแล้ว','success');
      }catch(e){flash((e as Error).message,'error');}
      return;
    }
  ...
  async function deleteRecord(record:Receipt){
    ...
    try{
      ...
      setHistoryRecords(previous=>previous.filter(item=>item.id!==record.id));
      flash('ลบรายการรับเข้าแล้ว','success');
    }catch(e){flash((e as Error).message,'error');}
    finally{setDeletingId('');}
  }
  function startEditRecord(record:Receipt){
    if(!user||!(user.role==='admin'||record.employee===user.username)||record.status==='duplicate')return;
    if(record.status==='pending'&&record.employee!==user.username){flash('รายการรอส่งต้องแก้ไขด้วยผู้ส่งเดิม','error');return;}
    setEditingRecord(record);setRawQR(record.rawQR);setRecognized(true);setFields({...empty,...record});setManual(false);setManualText('');setView('scan');setNotice(null);
  }
  function cancelEdit(){
    setEditingRecord(null);setRawQR('');setRecognized(false);setFields({...empty,warehouse:fields.warehouse,location:fields.location});setNotice(null);
  }
  const historyMap=new Map<string,Receipt>(historyRecords.map(item=>[item.id,item]));
  records.forEach(item=>{const remote=historyMap.get(item.id);if(!remote||item.status==='pending')historyMap.set(item.id,item);});
  const historyItems=[...historyMap.values()].sort((a,b)=>new Date(b.createdAt).getTime()-new Date(a.createdAt).getTime());
  const historyEmployees=[...new Set(historyItems.map(item=>item.employee).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  const filteredHistory=historyItems.filter(item=>{const itemDate=dateKey(item.createdAt);return (!historyStartDate||itemDate>=historyStartDate)&&(!historyEndDate||itemDate<=historyEndDate)&&(!historyEmployee||item.employee===historyEmployee);});
  ...
  return <div className="app-shell">
    ...
    {view==='scan'&&<>
      ...
      <div className="receiving-grid"><section className="panel scan-panel"><div className="panel-heading"><h2><span className="step-number">01</span>สแกน QR code</h2><span className="micro-label">PART LABEL</span></div>
      ...
      </section><section className="panel detail-panel"><div className="panel-heading"><h2><span className="step-number">02</span>{editingRecord?'แก้ไขรายการ':'ตรวจสอบและรับเข้า'}</h2><span className={'pill '+(rawQR?'ready':'')}>{editingRecord?'กำลังแก้ไข':rawQR?'อ่านรหัสแล้ว':'รอสแกน'}</span></div>
        {!rawQR?<div className="empty-detail"><div><Box size={39} strokeWidth={1.3}/></div><h3>ข้อมูลพาร์ทจะแสดงที่นี่</h3><p>เปิดกล้องสแกน QR บนป้ายพาร์ท<br/>หรือเลือกกรอกรหัสเองเพื่อเริ่มรับเข้า</p><span><ScanLine size={15}/> สแกน <ArrowRight size={14}/> ตรวจสอบ <ArrowRight size={14}/> รับเข้า</span></div>:<form ref={form} className="receipt-form" onSubmit={submit}>
          ...
          <div className="form-grid">{field('materialCode','Material code',true)}{field('lotNumber','Lot number',true)}{field('materialName','Material name')}{field('specification','Specification')}{warehouseField}{locationField}{field('quantity','จำนวนรับจริง',true,'0','number')}{field('unit','หน่วย',true)}<label>ชื่อ / รหัสผู้รับ <span className="required">*</span><input required readOnly value={editingRecord?.employee||user.username}/><small>ใช้ username จากการ Login อัตโนมัติ</small></label>{field('notes','หมายเหตุ')}</div>
          <div className="form-actions"><button type="button" className="text-button muted" onClick={cancelEdit}>ยกเลิก</button><button className="primary-button" disabled={sending}>{sending?<LoaderCircle className="spin" size={19}/>:editingRecord?<Pencil size={19}/>:<PackageCheck size={19}/>} {editingRecord?'บันทึกการแก้ไข':'ยืนยันรับเข้าคลัง'}<ArrowRight size={18}/></button></div>
        </form>}
      </section></div>
    ...
    {view==='history'&&<section className="panel"><div className="panel-heading"><h2>รายการรับเข้าทั้งหมด <span className="muted">({filteredHistory.length})</span></h2><div className="history-heading-actions"><button className="text-button" onClick={()=>{setHistoryLoaded(false);void loadHistory();}} disabled={historyLoading}><RotateCw size={16} className={historyLoading?'spin':''}/>รีเฟรช</button><button className="secondary-button" onClick={syncAll} disabled={sending||!pending.length||!online}><RotateCw size={16} className={sending?'spin':''}/>{sending?'กำลังส่ง…':'ส่งรายการรอ'}</button></div></div><div className="history-filters"><label><Filter size={15}/>วันที่เริ่มต้น<input type="date" value={historyStartDate} max={historyEndDate||undefined} onChange={e=>{const next=e.target.value;setHistoryStartDate(next);if(next&&historyEndDate&&next>historyEndDate)setHistoryEndDate(next);}}/></label><label>วันที่สิ้นสุด<input type="date" value={historyEndDate} min={historyStartDate||undefined} onChange={e=>{const next=e.target.value;setHistoryEndDate(next);if(next&&historyStartDate&&next<historyStartDate)setHistoryStartDate(next);}}/></label><label>ผู้ลงข้อมูล<select value={historyEmployee} onChange={e=>setHistoryEmployee(e.target.value)}><option value="">ทั้งหมด</option>{historyEmployees.map(name=><option key={name} value={name}>{name}</option>)}</select></label><button type="button" className="secondary-button" onClick={()=>{setHistoryStartDate('');setHistoryEndDate('');setHistoryEmployee('');}}>ล้างตัวกรอง</button></div><p className="history-note">โหลดรายการล่าสุดจาก Google Sheets ได้ถึง 2,000 รายการ · รายการรอส่งเก็บอยู่ในเบราว์เซอร์นี้จนกว่าจะส่งสำเร็จ</p>{historyLoading&&!historyItems.length?<div className="empty-history"><LoaderCircle className="spin" size={24}/>กำลังโหลดรายการ…</div>:filteredHistory.length?renderRows(filteredHistory):<div className="empty-history"><History size={28}/>{historyItems.length?'ไม่พบรายการตามตัวกรอง':'ยังไม่มีรายการ'}</div>}</section>}
  ...
  function renderRows(items:Receipt[]){return <div className="records-list"><div className="record-table-header" aria-hidden="true"><span>พาร์ท / รายละเอียด</span><span>จำนวน</span><span>สถานะ</span><span>จัดการ</span></div>{items.map(r=><div className="record-row" key={r.id}><span className="record-icon"><Box size={21}/></span><div className="record-main"><strong title={r.materialCode}>{r.materialCode}</strong><span title={`Lot ${r.lotNumber} · ${r.location} · ${r.employee}`}>Lot {r.lotNumber} · {r.location} · {r.employee}</span>{r.error&&<small className="error-text">{r.error}</small>}</div><div className="record-quantity"><strong>{r.quantity} <small>{r.unit}</small></strong><span>{date(r.createdAt)}</span></div><span className={'record-status '+r.status}>{r.status==='synced'?<><Check size={14}/>บันทึกแล้ว</>:r.status==='duplicate'?'รายการซ้ำ':<><RotateCw size={13}/>รอส่ง</>}</span>{user&&(user.role==='admin'||r.employee===user.username)&&r.status!=='duplicate'&&<div className="record-actions"><button type="button" className="record-edit" aria-label={`แก้ไขรายการ ${r.materialCode}`} onClick={()=>startEditRecord(r)}><Pencil size={15}/>แก้ไข</button><button type="button" className="record-delete" aria-label={`ลบรายการ ${r.materialCode}`} disabled={deletingId===r.id} onClick={()=>void deleteRecord(r)}>{deletingId===r.id?<LoaderCircle className="spin" size={15}/>:<Trash2 size={15}/>}ลบ</button></div>}</div>)}</div>;}
}
